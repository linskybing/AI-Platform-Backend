...
import (
	...
	"github.com/linskybing/platform-go/pkg/storage"
...
func UploadObject(ctx context.Context, objectName string, contentType string, contentReader io.Reader, contentSize int64) error {
	...
	_, err := storage.Client.PutObject(ctx, storage.BucketName, objectName, contentReader, contentSize, minioSDK.PutObjectOptions{
...
func DownloadObject(ctx context.Context, objectName string) ([]byte, error) {
	obj, err := storage.Client.GetObject(ctx, storage.BucketName, objectName, minioSDK.GetObjectOptions{})
	if err != nil {
		return nil, err
	}
	defer obj.Close()
...
func DeleteObject(ctx context.Context, objectName string) error {
	return storage.Client.RemoveObject(ctx, storage.BucketName, objectName, minioSDK.RemoveObjectOptions{})
}
