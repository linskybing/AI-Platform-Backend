...
var CreateByJson = func(jsonStr []byte, ns string) error {
	if k8s.Mapper == nil || k8s.DynamicClient == nil {
	...
	mapping, err := k8s.Mapper.RESTMapping(gvk.GroupKind(), gvk.Version)
	if err != nil {
		return err
	}
	...
	resourceClient := k8s.DynamicClient.Resource(mapping.Resource).Namespace(ns)
...
func ExpandPVC(ns, pvcName, newSize string) error {
	...
	client := k8s.Clientset.CoreV1().PersistentVolumeClaims(ns)
...
var CreatePV = func(name string, storageClassName string, size string, path string) error {
	...
	_, err = k8s.Clientset.CoreV1().PersistentVolumes().Create(context.TODO(), pv, metav1.CreateOptions{})
	if err != nil {
...
var CreateHubPVC = func(ns string, name string, storageClassName string, size string) error {
	...
	_, err = k8s.Clientset.CoreV1().PersistentVolumeClaims(ns).Create(context.TODO(), pvc, metav1.CreateOptions{})
...
var CreateNFSDeployment = func(ns string, pvcName string) error {
	...
	_, err := k8s.Clientset.AppsV1().Deployments(ns).Create(context.TODO(), deploy, metav1.CreateOptions{})
	if err != nil && !apierrors.IsAlreadyExists(err) {
